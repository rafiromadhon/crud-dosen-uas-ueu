...
define("db_password", "");
define("db_name", "uas_pemob");
define("base_url","http://localhost/uas_pemob/crud/");
	...
	if ($bCheckSession == 0){
		...
		if ($_cmd == "check_session"){ //. untuk cek session autologin
			...
			if ($conn){
				$response["status"] = 1;
				$response["desc"] = "Success.";
				$response["data"] = get_all_data_dosen($conn, $_search);
			}else{
				$response["status"] = 0;
				$response["desc"] = "Database error.";
			}
			...
			if ($conn){
				$response["status"] = 1;
				$response["desc"] = "Success.";
				$response["data"] = get_data_dosen_by_id($conn, $_id);
			}else{
				$response["status"] = 0;
				$response["desc"] = "Database error.";
			}
			...
			if ($conn){
				$b = insert_data_dosen($conn, $_nama, $_alamat, $_jk);
				if ($b){
					$response["status"] = 1;
			...
			if ($conn){
				$b = delete_data_dosen_by_id($conn, $_id);
				if ($b){
					$response["status"] = 1;
			...
			if ($conn){
				$b = update_data_dosen($conn, $_nama, $_alamat, $_jk, $_id);
				if ($b){
					$response["status"] = 1;
...
function get_all_data_dosen($conn, $search){
	$ret = [];
...
function get_data_dosen_by_id($conn, $id){
	$ret = [];
...
function insert_data_dosen($conn, $_nama, $_alamat, $_jk){
	$ret = false;
	...
	$query = "insert into dosen(nama, alamat, jk) values ('$_nama','$_alamat','$_jk')";
	if ($conn){
...
function update_data_dosen($conn, $_nama, $_alamat, $_jk, $_id){
	$ret = false;
	$_id = mysqli_real_escape_string($conn, $_id);
	$_nama = mysqli_real_escape_string($conn, $_nama);
	$_alamat = mysqli_real_escape_string($conn, $_alamat);
	$_jk = mysqli_real_escape_string($conn, $_jk);
	$query = "update dosen set nama = '$_nama', alamat = '$_alamat', jk = '$_jk' where id = '$_id'";
	if ($conn){
        $ret = mysqli_query($conn, $query);
		//print( $result);
    }
...
function delete_data_dosen_by_id($conn, $id){
	$ret = false;
	$id = mysqli_real_escape_string($conn, $id);
