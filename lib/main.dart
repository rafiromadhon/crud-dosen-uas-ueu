...
import 'package:crud_dosen_v2/login.dart';
import 'package:crud_dosen_v2/upload.dart';
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      routes: {
        ...
        '/crud': (_) {
          return MyHomePage(title: 'uas_pemob CRUD Demo');
        },
...
class DialogEditDosen extends StatefulWidget {
  final String title;
  final Map rowDosen;
  final String action; //. pembeda antara "edit" / "new"
  DialogEditDosen({Key key, this.title, this.rowDosen, this.action})
      : super(key: key);

  @override
  _DialogEditDosenState createState() => _DialogEditDosenState();
}

class _DialogEditDosenState extends State<DialogEditDosen> {
  ...
  void initState() {
    ...
    //_txtID.text = widget.rowDosen["id"].toString();
    if (widget.action == "edit") {
      // jika edit, isi nilai control2 nya
      _txtNama.text = widget.rowDosen["nama"].toString();
      _txtAlamat.text = widget.rowDosen["alamat"].toString();
      _valSelectedJK = widget.rowDosen["jk"].toString();
    }
    super.initState();
  }
    ...
    return AlertDialog(
      ...
      actions: <Widget>[
        FlatButton(
          ...
          onPressed: () {
            String _id = isEdit() ? widget.rowDosen["id"].toString() : "";
            ...
            util
                .showAlert(context, "Are you sure ?",
                    isEdit() ? "Update Data, ID = ($_id)" : "Insert New Data")
                .then((b) {
              if (b == true) {
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _showNewDialog() {
    showDialog(
        ...
        builder: (BuildContext context) {
          return DialogEditDosen(
              rowDosen: null, title: "Tambah Dosen", action: "new");
        }).then((ret) {
      //. format kembalian (status#desc)
      if (ret != null && ret.toString().length > 0) {
        List<String> lstRet = ret.toString().split("#");
  ...
  List<Widget> _getRows() {
    List<Widget> lstRow = [];
    for (int i = 0; i < _lstDataDosen.length; i++) {
      dynamic _r = _lstDataDosen[i];
      lstRow.add(Container(
        decoration: BoxDecoration(
          ...
          color: (i % 2) == 0
              ? Colors.white
              : Colors.black.withOpacity(0.1), //. odd event row
        ),
        child: Row(
          children: <Widget>[
            Container(
